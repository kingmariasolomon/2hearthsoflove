...
class Router
{
    public static function route($url) {
        ...
        if(method_exists($controller, $action)){    //cheks if action exists in the controller class
            ...
            // $dispatch->$action($queryParams); THIS LINE IS AN EXAMPLE
        }else{
    ...
    public static function hasAccess($controller_name, $action_name='index') {
        ...
        $acl = json_decode($acl_file, true);  $current_user_acls = ["Guest"];
        $grantAccess = false;
        if(Session::exists(CURRENT_USER_SESSION_NAME) && Session::exists(ADMIN_SESSION_NAME)) {
            $current_user_acls[] = self::setUserLevel(Session::get(ADMIN_SESSION_NAME));
    ...
    private static function setUserLevel($ULevel){
        $ul = "";
        switch ($ULevel) {
            case 1:
                $ul = "LoggedIn";
