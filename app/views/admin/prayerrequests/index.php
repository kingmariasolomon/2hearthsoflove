...
<section class="uk-section uk-section-xsmall uk-dark">
    <div class="section-heading uk-margin-bottom">
        <div class="uk-container">
            <h2 class="uk-heading-bullet"><span class="txt-grad3"> Prayer Requests</h2>
            ...
            <div class="uk-grid-small uk-child-width-1-1" uk-grid>
                ...
                <?php if(!empty($this->reqests)): ?>
                    <table class="uk-table uk-table-divider uk-table-striped uk-table-hover">
                        ...
                        <tbody>
                        <?php foreach($this->reqests as $key => $val): ?>
                            <tr>
                                <td>
                                ...
                                <td><?=$val->email;?></td>
                                <td><?=$val->country;?></td>
                                <td id="switchElement" value="<?=$val->acknowledge?>" url="prayerrequest/acknowledgReqest/<?=$val->id?>"><?=($val->acknowledge==1)? "Acknowledge": "Waiting";?> </td>
                                <td><?=date("Y-m-d h:i A", strtotime($val->date));?></td>
                                <td>
                                    <a href="<?=PROOT?>prayerrequest/reqestDetails/<?=$val->id?>" class="uk-text-success no_link">
                                        <i class="fas fa-newspaper"></i> <sub><small> Details</small></sub> 
                                    </a>
                                    <a href="<?=PROOT?>prayerrequest/deleteReqest/<?=$val->id?>" class="uk-text- no_link" onclick="if(!confirm('Are You Sure?')){ return false;}">
...
<!-- <div class="circle-btn uk-position-fixed uk-position-medium uk-position-bottom-right">
    <a href="<?=PROOT?>prayerrequest/addnewbranch" class="uk-button uk-button-text no_link" type="submit" uk-tooltip="title: Add New; pos:left">
...
<script>
    $(document).ready(function () {
        ...
        roundSwitcher("round", function(e, element) {
            ...
            var datam = (e.target.checked)? 1 : 0;
            var data = new FormData();
            data.append("acknowledge", datam);
