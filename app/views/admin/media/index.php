                    ...
                    <table class="uk-table uk-table-divider">
                        ...
                        <?php foreach($this->medias as $key => $val): ?>
                            <tr>
                                ...
                                <td><img class="media" width="200" src="<?=ABS_PATH.$val->media;?>" alt="blog image"></td>
                                <td><?=$val->title;?></td>
                                <td><?=date("Y-m-d h:i A", strtotime($val->media_date));?></td>
                                <td>
                                    <a href="<?=PROOT?>media/editMedia/<?=$val->id?>" class="uk-text-success no_link">
                                        <i class="uk-icon" uk-icon="icon: pencil"></i> <sub><small> Edit</small></sub> 
                                    </a>
                                    <a href="<?=PROOT?>media/deleteMedia/<?=$val->id?>" class="uk-text-danger no_link" onclick="if(!confirm('Are You Sure?')){ return false;}">
...
<div class="circle-btn uk-position-fixed uk-position-medium uk-position-bottom-right">
    <a href="<?=PROOT?>media/addMedia" class="uk-button uk-button-text no_link" type="submit" uk-tooltip="title: Add New; pos:left">
    ...
    <div uk-drop="animation: uk-animation-slide-bottom-small; duration: 1000;offset: 5;pos: top-right">
        <ul class="uk-list uk-list-large uk-margin-right uk-text-right" >
            <a id="delete_all" class="no_link uk-box-shadow" href="#" uk-toggle uk-tooltip="title: Delete; pos:left">
                <span class="uk-icon uk-text-primary uk-margin-left" uk-icon="icon:trash; ratio:1.1"></span>
            </a>
        </ul>
    </div>
</div>
...
<script>
    $(document).ready(function(){
        ...
        $("#delete_all").click(function (e) {
            ...
            $.ajax({
                url : PROOT+"media/deletemultipleMedias/delete_data",
