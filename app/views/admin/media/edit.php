...
<section class="uk-section uk-section-xsmall uk-dark">
    <div class="section-heading uk-margin-bottom">
        <div class="uk-container">
            <h2 class="uk-heading-bullet"><span class="txt-grad3">Editing Media Clip</h2>  
        </div>
    </div>
                ...
                <div class="uk-grid-small" uk-grid>
                    <div class="uk-width-2-3">
                        <div class="uk-card uk-card-default uk-card-body uk-border-rounded">
                            <?= FH::inputBlock('text', 'Media Title', 'title',$medias->title, ['class'=>'uk-form-controls'], ['class'=>'uk-form-label'], ['class'=>'uk-input uk-border-rounded'], ['class'=>'uk-margin']);?>
                            <?= FH::textAreaBlock('Your message', 'description',$medias->description, ['class'=>'uk-form-label'], ['class'=>'uk-textarea uk-border-rounded', 'rows'=>'5', 'placeholder'=>'Enter Your Message...'], ['class'=>'uk-margin']);?>
                        </div>
                    </div>
                    <div class="uk-width-1-3">
                        <div class="uk-card uk-card-default uk-card-body uk-border-rounded">
                            <?= FH::selectBlock("Choose A Category", 'cat_id', $this->cat, ["class"=>"uk-form-controls"], $labelAttrs=[], ["class"=>"uk-select"], ["class"=>"uk-margin"], $this->cat[$medias->cat_id]);?>

                            <?= FH::selectBlock("Choose Sub Category", 'sub_cat_id', $this->sub, ["class"=>"uk-form-controls"], $labelAttrs=[], ["class"=>"uk-select"], ["class"=>"uk-margin-medium"], $this->sub[$medias->sub_cat_id]);?>

                            <?= FH::inputBlock('date', 'Date Of Event', 'media_date', $medias->media_date, ['class'=>'uk-form-controls'], ['class'=>'uk-form-label'], ['class'=>'uk-input uk-border-rounded'], ['class'=>'uk-margin']);?>
                        </div>
                    </div>
                    <div class="uk-width-1-1">
                        <div class="uk-card uk-card-default uk-card-body uk-border-rounded">
                            ...
                            <img id="piximg" abspath="<?=ABS_PATH?>" src="<?=ABS_PATH.$medias->media?>" alt="blog image">
                            </div>
                        </div>
                    </div>
                </div>
