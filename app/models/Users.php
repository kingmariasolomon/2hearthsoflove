...
{
    private $_isLoggedIn, $_sessionName, $_sessionAdmin, $_cookieName, $_softdelete, $_confirm;
    public static $currentLoggedInUser = null;
    public $id, $username, $email, $password, $fname, $pix="uploads/person/avatar.png", $lname, $acl, $deleted = 0, $access = 1;

    public function __construct($user='') {
        $table = 'users';
        parent::__construct($table);
        $this->_sessionName = CURRENT_USER_SESSION_NAME;
        $this->_sessionAdmin = ADMIN_SESSION_NAME;
    ...
    public function findUserById($id, $params=[]) {
        $conditions = [
            'conditions' => 'id = ?',
            'bind' => [$id]
        ];
    ...
    public function login($rememberMe=false, $isAdmin=false) {
        Session::set($this->_sessionName, $this->id);
        Session::set($this->_sessionAdmin, $isAdmin);//a check for admin validity should be ran here
        if($rememberMe) {
